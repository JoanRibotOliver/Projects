...
J = 41 # Anam canviant en apartat C)
...
# Llistes pel nombre d'iteracions als quals es vol representar el resultat del programa.
rlist = [0,1,10,100,1000,2000,3000,4000]
tlist = []

# Bucle per crear una llista de temps, per després representar la solució analítica.
for h in rlist:
    t = h*dt
    tlist.append(t)
...
if I == "a" or I == "a)" or I == "A":
    ...
    while t0 <= tf:
        ...
        # Calculam l'analítica en les iteracions que s'indiquen.
        if t0 in tlist:
            ...
            # I representam les dues solucions.
            plt.plot(x,u_a1)
            plt.xlabel("Eix x"), plt.ylabel("Solució u")
            
            plt.scatter(x,u,s=10,label=k)
